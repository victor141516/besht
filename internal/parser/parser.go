...
func (p *Parser) parsePrimary() (ast.Expression, error) {
	...
	case lexer.TokIdent, lexer.TokTypeString, lexer.TokTypeNumber, lexer.TokTypeBoolean, lexer.TokTypeStatus, lexer.TokTypeArray, lexer.TokFrom:
		...
		if name == "Object" && p.peekType() == lexer.TokDot && (p.peekN(1).Literal == "keys" || p.peekN(1).Literal == "values" || p.peekN(1).Literal == "entries" || p.peekN(1).Literal == "hasOwn" || p.peekN(1).Literal == "assign") {
			methodTok := p.peekN(1)
			p.advance()
			p.advance()
			args, err := p.parseArgList()
