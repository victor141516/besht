...
func TestValidator_ListToSpliced(t *testing.T) {
	mustCheck(t, `let l: Array<string> = ["a", "b", "c"]
let inserted: Array<string> = l.toSpliced(1, 0, "x")
let removed: Array<string> = l.toSpliced(1)`)
}
...
let cmp: number = s.localeCompare("banana")`},
		{"string trim aliases", `let s: string = "  hello  "
...
let ok: boolean = Object.hasOwn(user, "name")`},
		...
		{"Object.is primitives", `let ok: boolean = Object.is("Victor", "Victor")`},
...
let keys: string[] = Object.keys(copy(user))`},
		...
		{"primitive valueOf", `let text: string = "hello".valueOf()
let n: number = (40 + 2).valueOf()
let ok: boolean = true.valueOf()`},
...
let c: string = s.charAt()`, "charAt() takes 1 argument"},
		{"trimLeft arity", `let s: string = " abc "
let c: string = s.trimLeft("x")`, "trimLeft() takes no arguments"},
		{"valueOf arity", `let s: string = "abc"
...
let i: number = s.localeCompare()`, "localeCompare() takes 1 argument"},
...
let next: string[] = l.toSorted((a, b) => 0)`, "toSorted() takes no arguments"},
		{"list toSpliced arity", `let l: string[] = ["a"]
let next: string[] = l.toSpliced()`, "toSpliced() takes at least 1 argument"},
		{"string lastIndexOf arity", `let s: string = "abc"
...
let entries: string[][] = Object.entries(envObj)`, "Object.entries() requires an object literal or named object"},
		...
		{"Object.is arity", `let ok: boolean = Object.is("x")`, "Object.is() takes 2 arguments"},
...
func TestValidator_ListPredicateCallbacks(t *testing.T) {
	...
	let chars: string[] = items.flatMap(item => item.split(""))
