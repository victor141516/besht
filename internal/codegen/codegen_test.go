...
func TestCodegen_NumberParseIntHexByteSliceUsesTinyHelper(t *testing.T) {
	out := compile(t, `function parseHex(hex: string): number {
    return Number.parseInt(hex.slice(0, 2), 16)
}`)
	assertContains(t, out, `_bst_hex_byte()`)
	assertContains(t, out, `_bst_hex_byte "$_parseHex_hex" 0`)
	assertNotContains(t, out, `_bst_parse_int`)
	assertNotContains(t, out, `cut -c`)
}

func TestCodegen_PrefixStripTernaryUsesParameterExpansion(t *testing.T) {
	out := compile(t, `function clean(hex: string): string {
    return hex.startsWith("#") ? hex.slice(1) : hex
}`)
	assertContains(t, out, `"${_clean_hex#\#}"`)
	assertNotContains(t, out, `_bst_starts_with`)
	assertNotContains(t, out, `cut -c`)
}
...
let localeUpper: string = "hello".toLocaleUpperCase()
let localeLower: string = "HELLO".toLocaleLowerCase()
...
let legacySub: string = "hello".substr(1, 3)
let legacyTail: string = "hello".substr(-2)
...
let joined: string = "hello".concat(" ", "besht")`)
	...
	assertContains(t, out, `localeUpper='HELLO'`)
	assertContains(t, out, `localeLower='hello'`)
	assertContains(t, out, `left='hi  '`)
	assertContains(t, out, `right='  hi'`)
	assertContains(t, out, `sliced='ell'`)
	assertContains(t, out, `sub='ell'`)
	assertContains(t, out, `legacySub='ell'`)
	assertContains(t, out, `legacyTail='lo'`)
...
let sub = greeting.substr(1, 3)
let tail = greeting.substr(-2)
...
func TestCodegen_StringSubstrFallback(t *testing.T) {
	out := compile(t, `function part(s: string, start: number) {
    let a: string = s.substr(start)
    let b: string = s.substr(-2, 1)
}`)
	assertContains(t, out, `_hasLength`)
	assertContains(t, out, `_length`)
	assertContains(t, out, `_a=_len+_a`)
	assertContains(t, out, `substr(_s,_a+1,_n)`)
}
...
let foundLast = items.findLast((x, i) => x == "a" && i >= 0)
let foundLastIndex = items.findLastIndex(x => x == "a")`)
	...
	assertContains(t, out, `_find_5_22_count=0`)
	assertContains(t, out, `_bst_expr_5_22="$_BESHT_NULLISH_SENTINEL"`)
	assertContains(t, out, `_bst_expr_6_27=-1`)
	assertContains(t, out, `_BESHT_NULLISH_SENTINEL`)
	assertNotContains(t, out, `local `)
	assertNotContains(t, out, `[[`)
}
...
let before: number = "apple".localeCompare("banana")
let same: number = "apple".localeCompare("apple")
let after: number = "banana".localeCompare("apple")
let pos: number = "hello hello".indexOf("lo", 4)`)
	...
	assertContains(t, out, `before=-1`)
	assertContains(t, out, `same=0`)
	assertContains(t, out, `after=1`)
...
func TestCodegen_StaticStringVariableSearchMethods(t *testing.T) {
	out := compile(t, `let greeting = "hello"
let needle = "ell"
let has = greeting.includes(needle)
let sw = greeting.startsWith("he")
let ew = greeting.endsWith("lo")
let first = greeting.indexOf("l")
let last = greeting.lastIndexOf("l")
let cmp = greeting.localeCompare("world")
...
func TestCodegen_StaticStringVariableSearchFallbackAfterControlAssignment(t *testing.T) {
	out := compile(t, `let greeting = "hello"
while (true) {
    greeting = "bye"
    break
}
let has = greeting.includes("ell")
let cmp = greeting.localeCompare("hello")`)
	assertContains(t, out, `_bst_includes`)
	assertContains(t, out, `LC_ALL=C awk`)
	assertNotContains(t, out, `has=1`)
	assertNotContains(t, out, `cmp=0`)
}
...
let cmp: number = ("ap" + "ple").localeCompare("ban" + "ana")
...
func TestCodegen_StringCharAt(t *testing.T) {
	out := compile(t, `function pick(s: string) {
...
func TestCodegen_StringSubstring(t *testing.T) {
	out := compile(t, `function cutIt(s: string) {
