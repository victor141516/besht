...
- Native array-style APIs: indexing, `.at()`, `.length`, `.slice()`, `.push()`, `.concat()`, `.fill()`, default lexical `.sort()`, `.toReversed()`, `.includes()`, `.map()`, `.flatMap()`, `.filter()`, `.some()`, `.every()`, `.find()`, `.findIndex()`, `.findLast()`, `.findLastIndex()`, `.reduce()`, `.reduceRight()`, and statement-position `.forEach()`.
