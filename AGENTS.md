...
Array `sort()` is the narrow default lexical slice only: no comparator callback support. Static scalar receivers and static scalar method chains should fold with the same constant-array machinery as `reverse()` and `toReversed()`. Dynamic scalar receivers lower to `LC_ALL=C sort`. Keep Besht's existing array-returning convention: value-position `let sorted = items.sort()` returns a sorted value, while statement-position `items.sort()` rebinds the named receiver to that result. `toReversed()` is the copy-style API and must not be added to statement-position receiver rebinding.
