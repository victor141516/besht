...
**Arrow functions are callback-capable function values.** Direct list `.map(x => expr)`, `.map((x, i) => { return expr })`, `.filter((x, i) => truthyExpr)`, `.some((x, i) => truthyExpr)`, `.every((x, i) => truthyExpr)`, `.find((x, i) => truthyExpr)`, `.findIndex((x, i) => truthyExpr)`, `.reduce((acc, cur) => { ... }, init)`, and statement-position `.forEach((x, i) => { ... })` callbacks are supported, and arrows can also be assigned to variables, passed to functions, called as `cb(value)`, and passed to list callback methods as stored callbacks. Parenthesized arrows may carry TypeScript-shaped return hints: `(x: string): string => x`. `ast.TypeFunction` is a representation hint only; do not add type mismatch checks. Arrow value declarations lower to generated shell functions named `_bst_arrow_<binding>_<line>_<col>`, store that shell function name in the binding, and name params as `<function_name>_param_<param_name>`; never emit `local`.
...
Stored callback values are invoked through their shell function names. List `.map()`, `.filter()`, `.some()`, `.every()`, `.find()`, `.findIndex()`, and scalar/list accumulator `.reduce()` pass `(item, index)` or `(accumulator, current)` as shell arguments; one-parameter callbacks simply ignore the extra index. Statement-position `.forEach(storedCallback)` invokes the function directly in the current shell so outer assignments and `Set.add()` side effects persist. Value-position calls such as `let x = cb(value)` still use the same stdout-return command-substitution model as named Besht functions, so do not promise mutation persistence for every value-position callback call until a broader return-slot protocol exists. Object-accumulator `reduce()` remains on the direct-arrow path because compiler-managed object slot mutations must happen in the current shell.
