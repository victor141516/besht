...
Prefer native array APIs for new user-facing examples and compiler work: `items.length`, `items[0]`, `items.slice(1)`, `items.push(value)` or `[...items, value]`, `items.includes(value)`, `items.concat(other)`, `items.sort()`, and `items.toSorted()`. The old global list helpers `len`, `head`, `tail`, `append`, `contains`, and `concat` remain supported for compatibility, but do not add new global list helpers.
...
**Prefer native array APIs over global list helpers.** New language work, examples, and docs should use `items.length`, `items[0]`, `items.slice(1)`, `items.push(value)` or `[...items, value]`, `items.includes(value)`, `items.concat(other)`, `items.sort()`, and `items.toSorted()`. The old global helpers `len`, `head`, `tail`, `append`, `contains`, and `concat` remain supported for compatibility, but do not add new global list helpers or present them as the primary API.
