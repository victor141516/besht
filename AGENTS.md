...
Static boolean `console.log()` and `console.error()` arguments such as `Boolean("")`, `true`, and simple static `!`/`&&`/`||` expressions render directly as `true`/`false`; static boolean `if` and ternary conditions use the same generator-aware constant path. Dynamic boolean console arguments should reuse `genCondition()` directly: single-argument console calls emit `if <condition>; then printf '%s\n' true; else printf '%s\n' false; fi`, while multi-argument console calls use one `$(if <condition>; then printf true; else printf false; fi)` argument. Do not compute a `1`/`0` boolean capture and then wrap it in a second formatter when the condition is available.
