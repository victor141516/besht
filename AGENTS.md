...
let objectSameName: boolean = Object.is(user.name, "Ada")
...
let sameName: boolean = Object.is(user.name, "Updated")
...
let legacySub: string = name.substr(0, 5)
...
**Skill validation should cover control-flow and status idioms.** `node-eq/tests/commands/skill_control_flow_status.bsh` is paired with a shell source that uses `case`, loops, `break`/`continue`, expected command failures, `$?`, and `exit`. The Besht fixture intentionally uses `switch`, native arrays, `break`/`continue`, named command objects, `.exitCode()`, `status.toString()`, and `process.exit()`; keep it as a guardrail against agents wrapping shell control flow in `sh -c` or using `try/catch` for command statuses that are expected data.
...
**Declaration-file skill validation is compiler-test-backed.** `TestIntegration_ImportedDeclarationDescribesExternalRuntimeFunction` covers imported `.d.bsh` signatures for external runtime functions. Declaration files provide editor/compiler signatures only: they do not emit wrappers, source shell files, or make runtime implementations appear. Keep node-eq fixtures away from `.d.bsh` imports unless the parity runner grows declaration-aware module handling.
...
`Object.is(a, b)` is a scalar primitive comparison builtin. Static calls over strings, numbers, booleans, `null`, and `undefined` fold to constants with type tags so `Object.is(true, 1)` is false and `Object.is(null, undefined)` is false. Dynamic scalar values compare both inferred primitive kind and generated value. Reject statically known arrays, objects, Sets, commands, fetch responses, and JSON values until Besht has a richer object identity model. Do not claim support for JavaScript's `NaN` or `-0` SameValue edge cases because Besht does not currently represent them distinctly.
