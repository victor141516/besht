...
description: >
  ...
  array/string/number methods, Array.from({ length }), Array.of(), Array.isArray(), Object.keys(), Object.values(), Object.entries(), Object.hasOwn(), Object.assign(), JSON.parse(), JSON.stringify(), Set<T>, nested arrays, object literals, classes, getters/setters, logical operators, nullish coalescing ??, Besht.args.argv()/positional()/option()/flag(), string
...
Static scalar array literals and variables bound to static scalar arrays fold `.includes()`, `.indexOf()`, and `.lastIndexOf()` calls to constants when the needle is static. Dynamic array searches keep the POSIX `grep`/`awk` path.
