...
console.log(["a", "b"]); // [ a, b ]; static scalar array output compiles to one printf
...
Arrow functions can be stored in variables, passed to functions, called as function values, and passed to array callback APIs. Direct arrows support expression-bodied and block-bodied forms for array `.map()`, `.flatMap()`, `.filter()`, `.reduce()`, `.reduceRight()`, and statement-position `.forEach()`. Scalar-array `.some()`, `.every()`, `.find()`, `.findIndex()`, `.findLast()`, and `.findLastIndex()` may use direct arrow predicates or stored callbacks.

When translating shell pipelines that transform already-known text or numbers, prefer native Besht arrays and methods over external `sed`/`awk`/`grep`/`tr` commands. For example, a shell pipeline that trims lines, filters by prefix, uppercases, joins labels, sums numbers, reverses an accumulation, or prints `NR - 1` indexes is usually clearer as `items.map(...)`, `filter(...)`, `word.trim()`, `startsWith(...)`, `toUpperCase()`, `join(...)`, `reduce(...)`, `reduceRight(...)`, and `forEach((item, index) => ...)`. Reserve command pipelines for real external data sources or operations that need an external tool.

For literal delimiter-separated records or shell snippets that use `awk -F:`, `cut`, `paste`, or repeated membership probes over a static table, model the data directly. Use object literals for key/value records, object spread, `Object.assign()`, or `Object.fromEntries()` for scalar-safe copies/merges, `Object.keys()`, `Object.entries()`, and `Object.hasOwn()` for enumeration and probes; use `Set<T>` for membership groups and `JSON.stringify()` for JSON output instead of preserving the text-processing pipeline.
...
let chosenIsArray = Array.isArray(chosen)
let initials = ""
...
let reversed = nums.reduceRight((acc, n) => acc + n.toString(), "")
...
`Array.from("abc")` creates a character array. `Array.from({ length })` differs from JavaScript: it creates `[0, 1, ... length - 1]` and does not support general iterables or mapper callbacks. `Array.of(...)` creates an array from the given values. `Array.isArray(value)` is a static predicate: it returns true for values Besht can infer as arrays and false otherwise, without runtime shape metadata. Use `items.at(-1)` for the last item or other JavaScript-style positive/negative index reads; out-of-range `at()` results are nullish, so `items.at(99) ?? "missing"` works. `.fill(value, start?, end?)` returns a scalar array with JavaScript-style bounds; value-position calls leave the receiver unchanged, while statement-position `items.fill(...)` rebinds the named array to the filled result. `.map()` and `.flatMap()` support expression or block bodies and one or two direct arrow parameters: `(item)` or `(item, index)`. `return` inside a block-bodied `.map()` or `.flatMap()` callback emits that mapped value for the current item and continues the loop. Block-bodied callbacks for these methods currently support `return`, `if`/`else`, and assignment statements; arbitrary expression statements are rejected. `.flatMap()` flattens one callback-returned scalar-array level; scalar callback returns behave like `.map()`. `.filter()`, `.some()`, `.every()`, `.find()`, `.findIndex()`, `.findLast()`, and `.findLastIndex()` use JavaScript-style truthiness and may receive `(item, index)`. `.some(callback)` returns true for the first truthy callback result and false for an empty array. `.every(callback)` returns false for the first falsey callback result and true for an empty array. `.find(callback)` returns the first matching scalar element; `.findLast(callback)` scans from the end and returns the last matching scalar element. Both return nullish when no item matches so `??` fallbacks work. `.findIndex(callback)` and `.findLastIndex(callback)` return the zero-based matching index or `-1`. `.reduce()` and `.reduceRight()` take a 2-parameter callback plus an initial value; direct arrows and stored callbacks support scalar/array accumulators, stored Besht callbacks can mutate object accumulators, and `.reduceRight()` visits elements from right to left. `.forEach()` is statement-only, takes a direct arrow or stored callback with `(item)` or `(item, index)`, compiles static scalar direct-arrow receivers to compact `for` loops, preserves outer assignment and `Set.add()` side effects, and rejects direct-arrow callback `return`, `break`, `continue`, and pure value expressions. Type assertions such as `[] as string[]` are erased and are useful for empty array accumulators. Array literal spread such as `[...items, extra]` is supported generically. Returned arrows capture independent state, callback factories can be passed directly to array methods, and direct function-value calls in value position preserve captured mutations. Array callback expressions used in value or condition position preserve assignment, `Set.add()`, and returned-closure side effects after `.map()`, `.flatMap()`, `.filter()`, `.some()`, `.every()`, `.find()`, `.findIndex()`, `.findLast()`, `.findLastIndex()`, and `.reduceRight()` complete.

Inline static scalar object literal `Object.keys()`, `Object.values()`, `Object.entries()`, and `Object.hasOwn()` calls compile to constants, as do static scalar primitive `Object.is()` calls. Unmutated named object `Object.keys()`, static-scalar `Object.values()`/`Object.entries()`, static-key `Object.hasOwn()`, literal-only `Object.assign({}, ...)`, literal-only `Object.fromEntries(...)`, and literal-only object spread calls also fold from compiler-managed static object metadata; mutated or dynamic objects keep metadata-backed output so assignments and spread copies stay visible.
...
Static ASCII string expressions built from literals, variables bound to static ASCII strings, concatenation, template interpolation, and chained transforms fold transforms such as `.trim()`, `.trimStart()`/`.trimLeft()`, `.trimEnd()`/`.trimRight()`, `.toUpperCase()`/`.toLocaleUpperCase()`, `.toLowerCase()`/`.toLocaleLowerCase()`, `.slice()`, `.substring()`, legacy `.substr()`, `.repeat()`, `.replace()`/`.replaceAll()`, `.concat()`, and `.padStart()`/`.padEnd()` with static arguments to constants. Dynamic and non-ASCII transforms use POSIX tools; dynamic string `slice()`, `substr()`, `at()`, and indexing support normal substring extraction. Locale case methods are compact aliases for the existing POSIX case transforms, not full locale-aware casing.

Simple prefix-strip ternaries such as `s.startsWith("#") ? s.slice(1) : s` compile compactly while keeping the same Besht behavior.

Static ASCII string literal `.split()` calls with static separators compile to constants when the resulting array elements contain no newlines; variables bound to static ASCII strings can use the same split folding. Dynamic and non-ASCII splits use POSIX tools.
...
`Set<T>` has `new Set<T>()`, `.has(value)`, and mutating `.add(value)`. Type parameters are annotations only. Straight-line static scalar adds and static membership checks compile compactly; dynamic values and callback/control-flow adds keep runtime membership checks. Nested array rows are preserved through `.map()` when the callback returns an array, and static nested indexes with known row and column fold to constants. `split("")` splits a string into characters. Tuple/array destructuring declarations evaluate the source once and assign each name from array indexes; static scalar array destructuring emits direct assignments. Optional chaining supports `obj?.prop`, `obj?.[key]`, `obj?.method()`, and nested chains; it short-circuits only on `null`/`undefined` and composes with `??`. General `fn?.()`, `obj.method?.()`, and optional assignment targets are not supported.
...
let sameName: boolean = Object.is(user.name, "Victor")
let copy: object = Object.assign({}, user, { active: false })
let rebuilt: object = Object.fromEntries(entries)
let merged: object = { ...user, active: false }
...
function withRole(obj: object, role: string): object {
    return { ...obj, role }
}
```
...
`JSON.stringify(value)` encodes `JSONValue`, strings, numbers, booleans, null/undefined, scalar arrays, and scalar-valued compiler-managed objects. It rejects commands, fetch responses, sets, and nested Besht objects/arrays that the current representation cannot preserve safely.
...
| `printf "$TEAM" \| awk -F: ...` over a literal table | object literals, `Set<T>`, `Object.entries()`, `Object.fromEntries()`, `Object.hasOwn()`, array callbacks, `JSON.stringify()` |
...
// Capture stdout -> array of lines
let logsCmd = $("find", "/var/log", "-name", "*.log")
logsCmd.run()
let logs: string[] = logsCmd.readStdoutLines()
...
TypeScript `for...of` loops are supported for array iteration and behave like Besht's value-iteration `for...in` loops.

Static scalar array expressions, static scalar `Array.of(...)` calls, static `Array.from({ length: N })` calls, and variables bound to them compile to compact shell `for` loops when elements do not contain newlines. Dynamic arrays use Besht's newline-safe read loop.
...
Prefer TypeScript-style array syntax and methods in new code:
...
`items.sort()` and `items.toSorted()` return default lexical sorted arrays. Static scalar receivers fold to constants; dynamic receivers use POSIX `LC_ALL=C sort`. Comparator callbacks such as `items.sort((a, b) => ...)` are not supported yet. `items.toReversed()`, `items.toSorted()`, and `items.toSpliced(start, deleteCount?, ...items)` are copy-style APIs and never rebind the receiver; statement-position `items.reverse()` and `items.sort()` are the Besht rebind forms. `toSpliced()` follows JavaScript-style start/delete bounds and folds for static scalar receivers. Like other Besht array-returning methods, `let sorted = items.sort()` leaves `items` unchanged, while a statement-position `items.sort()` rebinds the named array to the sorted result.
...
Static string literals, variables bound to static string literals, static scalar array expressions, and variables bound to static scalar arrays compile `.length` properties to numeric constants; dynamic lengths use POSIX `wc`.

Static scalar array literals and variables bound to static scalar arrays fold `.includes()`, `.indexOf()`, `.lastIndexOf()`, and `.at()` calls to constants when the needle or index is static. Dynamic array searches and `.at()` calls keep the POSIX `grep`/`awk` path.
